...
def allFilesWithSuffixs(directory, suffixs) : 
    """ List full paths of all files that end with suffix """ 
    return filter(lambda x : any(x.endswith(suffix) for suffix in suffixs), allfiles(directory))
...
def find_file_in_parents (file_base_name, start_path): 
    """
    Example: 

    >>> print(find_file_in_parents('args.pkl', 'iclight-env-map-eks02/version_3/checkpoints/epoch=1-step=59999.ckpt'))
    /sensei-fs-3/users/suchatur/expts/IC-Light/iclight-env-map-eks02/version_3/args.pkl
    """
    current_dir = osp.split(osp.abspath(start_path))[0] 
    while True :
        found_file = next(filter(lambda x : osp.split(x)[1] == file_base_name, listdir(current_dir)), None)
        if found_file is not None :
            return found_file
        if current_dir == '/' : 
            return None
        current_dir = osp.split(current_dir)[0] 
